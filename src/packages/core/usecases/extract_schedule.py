...
- intent が "smalltalk" または "unknown" の場合: reply は空文字列 "" でよい。

needs_web_search の判断ルール:
- ニュース・天気・株価・為替・試合結果・最新リリースなど、学習データ以降の事実を必要とする質問は true
- 予定管理・雑談・一般知識・過去の事実で答えられる場合は false
- intent が "add_schedule" または "list_schedule" の場合は必ず false
- 添付された画像の内容を問う質問（「これは何の画像？」「何が写っている？」「これは何？」など）は必ず false（モデルが画像を直接見て答えられる）"""
